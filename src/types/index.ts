...
/**
 * תגיות התנהגותיות — בנפרד מ"קטגוריה" (שם בוחרים ערך יחיד): כאן אפשר לסמן כמה תגיות
 * גם יחד באותה עסקה, כדי לראות בסטטיסטיקה אילו טעויות/תנאים חוזרים על עצמם הכי הרבה.
 */
export const TRADE_TAG_GROUPS = {
  'טעויות': [
    'נכנסתי מוקדם מדי (לפני אישור)',
    'הפוזיציה הייתה גדולה מדי',
    'נכנסתי בלי סטופ לוס מוגדר מראש',
    'סטיתי מהתוכנית שקבעתי מראש',
    'יצאתי מוקדם מדי מרוב פחד',
    'רדפתי אחרי המחיר (בלי סבלנות)',
    'נכנסתי מתוך תסכול אחרי הפסד קודם',
    'החזקתי בעסקה מפסידה יותר מדי זמן',
  ],
  'מצב שוק באותה עסקה': [
    'תנודתיות גבוהה באותו יום',
    'נפח מסחר נמוך מהרגיל',
    'יום דוחות/חדשות משמעותיות',
    'מגמת שוק חזקה וברורה',
    'שוק בדשדוש (בלי כיוון ברור)',
  ],
  'איכות ההזדמנות': [
    'הזדמנות מעולה (הכל התאים)',
    'עסקה מאולצת (בלי סיבה טובה להיכנס)',
    'עמדתי בכל התנאים של התוכנית',
  ],
} as const

export const ALL_TRADE_TAGS = Object.values(TRADE_TAG_GROUPS).flat()
...
export interface Position {
  ...
  tags: string[] // תגיות התנהגותיות (טעויות/מצב שוק/איכות ההזדמנות) — אפשר כמה יחד, מתוך TRADE_TAG_GROUPS
  followedPlan: boolean | null // "פעלתי לפי התוכנית?" — null = לא צוין
  tradeReview: string // סקירת עסקה חופשית: לקחים/טעויות/מה למדתי
}
